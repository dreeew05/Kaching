...
export default function CustomPressable({ text, onPress, disabled }: CustomPressableProps) {
  return (
    <Pressable
      className={`w-3/5 self-center rounded-full p-3 mb-5 ${disabled ? 'bg-gray' : 'bg-white'}`} // Use the disabled prop to conditionally apply styles
      onPress={disabled ? undefined : onPress} // Prevent onPress if disabled
      disabled={disabled} // Set the disabled prop for accessibility
    >
      <Text className={`text-green text-xl font-bold self-center ${disabled ? 'text-white' : ''}`}>{text}</Text>
    </Pressable>
  );
}
